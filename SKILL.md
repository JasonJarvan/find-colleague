...
| **某项目某段时间谁做了啥**（项目→人→已做工作，**带时间**） | `find-colleague worklog [--project X] [--since ...] [--until ...]` | `find-colleague worklog --project "EverOS Cloud" --since 2026-06-15` |
...
- **「X 项目这周/上个月谁做了啥」「某项目最近的产出/工时」**走 `worklog`：项目视角列「项目→人→
  已做工作」，可按**周**筛时间。`--project` 聚焦单项目（省略则全部项目分组）；`--since/--until`
  接受 `YYYY-MM-DD` / `YYYY-MM` / `YYYY-Www`，**按触及的周展开**（与某周周一–周日有交集即纳入）。
  「这周/上周/上个月」先换算成具体日期再传（粒度=周、周一起始）。无 `--exclude`（项目产出表非派活，
  通常不需排除本人；若用户明说排除自己再过滤展示）。
...
| `find-colleague worklog [--project X] [--since ...] [--until ...]` | 项目→人→已做工作（可按周筛时间） |
